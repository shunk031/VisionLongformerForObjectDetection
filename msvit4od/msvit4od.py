...
class Attention(nn.Module):
    def __init__(
        ...
        if rpe:
            ...
            self.local_relative_position_bias_table = nn.Parameter(
                torch.zeros((2 * wx - 1) * (2 * wy - 1), num_heads)
            )  # (2*wx-1, 2*wy-1, nH)
            trunc_normal_(self.local_relative_position_bias_table, std=0.02)
            if nglo >= 1:
                self.g2l_relative_position_bias = nn.Parameter(
                    torch.zeros(2, num_heads, nglo)
                )  # (2, nH, nglo)
                self.g2g_relative_position_bias = nn.Parameter(
                    torch.zeros(num_heads, nglo, nglo)
                )  # (nH, nglo, nglo)
                trunc_normal_(self.g2l_relative_position_bias, std=0.02)
                trunc_normal_(self.g2g_relative_position_bias, std=0.02)
    ...
    def forward(self, x, nx=None, ny=None):
        ...
        if self.rpe:
            assert (
                N == self.nglo + self.wx * self.wy
            ), "For relative position, N != self.nglo + self.wx*self.wy!"
            local_relative_position_bias = self.local_relative_position_bias_table[
                self.relative_position_index.view(-1)
            ].view(
                self.wx * self.wy, self.wx * self.wy, -1
            )  # Wh*Ww, Wh*Ww,nH
            relative_position_bias = local_relative_position_bias.permute(
                2, 0, 1
            ...
            if self.nglo > 0:
                ...
                global_relative_position_bias = torch.cat(
                    [
                        ...
                        self.g2l_relative_position_bias[0]
                        .unsqueeze(-1)
                        .expand(-1, -1, self.wx * self.wy),
                    ],
                    dim=-1,
                )  # nH, nglo, N
                ...
                local_relative_position_bias = torch.cat(
                    [
                        self.g2l_relative_position_bias[1]
                        .unsqueeze(1)
                        .expand(-1, self.wx * self.wy, -1),
...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        if ape:
            ...
            trunc_normal_(self.cls_pos_embed, std=0.02)
            trunc_normal_(self.x_pos_embed, std=0.02)
            trunc_normal_(self.y_pos_embed, std=0.02)
    ...
    def forward(self, xtuple):
        ...
        assert (
            nx <= self.nx and ny <= self.ny
        ), "Input size {} {} should <= {} {}!".format(nx, ny, self.nx, self.ny)
    ...
    def __init__(
        ...
        #         nglo=nglo, only_glo=only_glo)
            ...
            self.attn = PerformerSelfAttention(
                ...
                attn_drop=attn_drop,
                proj_drop=drop,
                nb_features=num_feats,
            )
        else:
            raise ValueError("Not supported attention type {}".format(attn_type))
...
class MlpBlock(nn.Module):
    ...
    def __init__(
        ...
        if out_dim is not None and out_dim != dim:
            self.shortcut = nn.Sequential(nn.Linear(dim, out_dim), nn.Dropout(drop))
...
class MsViT(Backbone):
    ...
    def __init__(
        ...
        **args,
        ...
        self.layer1 = self._make_layer(
            in_chans, self.layer_cfgs[0], dprs=dprs[0], layerid=1
        )
        ...
        self.layer2 = self._make_layer(
            self.layer_cfgs[0]["d"], self.layer_cfgs[1], dprs=dprs[1], layerid=2
        )
        ...
        self.layer3 = self._make_layer(
            self.layer_cfgs[1]["d"], self.layer_cfgs[2], dprs=dprs[2], layerid=3
        )
        ...
        if self.num_layers == 3:
            ...
            self.layer4 = self._make_layer(
                self.layer_cfgs[2]["d"], self.layer_cfgs[3], dprs=dprs[3], layerid=4
            )
            ...
            raise ValueError(
                "Numer of layers {} not implemented yet!".format(self.num_layers)
            )
        ...
        assert (
            self._size_divisibility == stride
        ), "Some stride down layer has been ignored!"
    ...
    def reset_vil_mode(self, mode):
        ...
        for longformer_attention in longformer_attentions:
            ...
            if mode_old != mode:
                ...
                logging.info(
                    "Change vil attention mode from {} to {} in "
                    "layer {}".format(mode_old, mode, longformer_attention)
                )
        return
    ...
    def _make_layer(self, in_dim, layer_cfg, dprs, layerid=0):
        ...
        assert (
            layerid == layer_id
        ), "Error in _make_layer: layerid {} does not equal to layer_id {}".format(
            layerid, layer_id
        )
...
def build_retinanet_msvit_fpn_backbone(cfg, input_shape: ShapeSpec):
    ...
    in_channels_p6p7 = bottom_up.output_shape()[
        cfg.MODEL.TRANSFORMER.OUT_FEATURES[-1]
    ].channels
    backbone = FPN(
        bottom_up=bottom_up,
        ...
        top_block=LastLevelP6P7(
            in_channels_p6p7, out_channels, cfg.MODEL.FPN.IN_FEATURES[-1]
        ),
