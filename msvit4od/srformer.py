...
class SRSelfAttention(nn.Module):
    def __init__(
        ...
        self.proj_sr = nn.Conv2d(
            dim, dim, kernel_size=rratio, stride=rratio, bias=False
        )
    ...
    def forward(self, x, nx=None, ny=None):
        ...
        keys, values = (
            kv[0],
            kv[1],
        )  # make torchscript happy (cannot use tensor as tuple) b x d x k
    ...
    def compute_macs(module, input, output):
        ...
        conv_per_position_flops = int(kernel_dims) * in_channels * filters_per_channel
