...
class SlidingChunk2D(torch.autograd.Function):
    ...
    def slidingchunk_qk(q_img: torch.Tensor, k_img: torch.Tensor, mode: int):
        ...
        if mode == 0:
            ...
            return einsum("b c m n l, b c m n t -> b m n l t", q_img, k_img) * 1.0
        else:
    ...
    def slidingchunk_av(attn: torch.Tensor, v_img: torch.Tensor, mode: int):
        ...
        if mode == 0:
            (
                ...
                attn10,
                attn11,
            ) = torch.split(attn, w2, dim=-1)
        elif mode == -1:
            attn00 = attn
        else:
            attn00, attnxx = torch.split(attn, w2, dim=-1)
        output = einsum("b m n l t, b c m n t -> b c m n l", attn00, v_img)  # 0,0

        if mode == 0:
            output = output + einsum(
                "b m n l t, b c m n t -> b c m n l",
                attnn1n1,
                torch.roll(v_img, shifts=(1, 1), dims=(2, 3)),
            )  # -1,-1
            output = output + einsum(
                "b m n l t, b c m n t -> b c m n l",
                attnn10,
                torch.roll(v_img, shifts=1, dims=2),
            )  # -1,0
            output = output + einsum(
                "b m n l t, b c m n t -> b c m n l",
                attnn11,
            ...
            output = output + einsum(
                "b m n l t, b c m n t -> b c m n l",
                attn0n1,
                torch.roll(v_img, shifts=1, dims=3),
            )  # 0,-1
            output = output + einsum(
                "b m n l t, b c m n t -> b c m n l",
                attn01,
            ...
            output = output + einsum(
                "b m n l t, b c m n t -> b c m n l",
                attnxx,
                torch.roll(v_img, shifts=shift, dims=(2, 3)),
            )  # 1,1
        else:
            output = output * 1.0
...
@lru_cache()
def _get_invalid_locations_mask_cyclic(
    nx: int, ny: int, padx: int, pady: int, w: int, device: str
):
    w2 = w ** 2
    mask = torch.BoolTensor(
        [
            [
                (
                    i // ny + (j // w2) // 3 == nx
                    and (nx - 1) * w + (j % w2) // w >= nx * w - padx
                )
                or (
                    i % ny + (j // w2) % 3 == ny
                    and (ny - 1) * w + (j % w2) % w >= ny * w - pady
                )
                for j in range(9 * w2)
            ]
...
@lru_cache()
def _get_invalid_locations_mask_zero(
    nx: int, ny: int, padx: int, pady: int, w: int, device: str
):
    w2 = w ** 2
    mask = torch.BoolTensor(
        [
            [
                i // ny + (j // w2) // 3 - 1 < 0
                or i // ny + (j // w2) // 3 - 1 >= nx
                or (i // ny + (j // w2) // 3 - 1) * w + (j % w2) // w >= nx * w - padx
                or i % ny + (j // w2) % 3 - 1 < 0
                or i % ny + (j // w2) % 3 - 1 >= ny
                or (i % ny + (j // w2) % 3 - 1) * w + (j % w2) % w >= ny * w - pady
                for j in range(9 * w2)
            ]
            for i in range(nx * ny)
        ],
...
@lru_cache()
def _get_invalid_locations_mask_exact(
    nx: int, ny: int, padx: int, pady: int, w: int, device: str
):
    w2 = w ** 2
    nx_max = nx * w - 1 - padx
    ...
    mask = torch.BoolTensor(
        [
            [
                [
                    (i // ny + (j // w2) // 3 - 1) * w + (j % w2) // w
                    < max(0, (i // ny - 1) * w + l // w)
                    or (i // ny + (j // w2) // 3 - 1) * w + (j % w2) // w
                    > min(nx_max, (i // ny + 1) * w + l // w)
                    or (i % ny + (j // w2) % 3 - 1) * w + (j % w2) % w
                    < max(0, (i % ny - 1) * w + l % w)
                    or (i % ny + (j // w2) % 3 - 1) * w + (j % w2) % w
                    > min(ny_max, (i % ny + 1) * w + l % w)
                    for j in range(9 * w2)
                ]
                for l in range(w2)
            ]
...
def slidingchunk_2dautograd(
    t1: torch.Tensor, t2: torch.Tensor, is_t1_diagonaled: bool = False, mode: int = 0
) -> torch.Tensor:
    if is_t1_diagonaled:
        return SlidingChunk2D.slidingchunk_av(t1, t2, mode)
    else:
